...
frontend_new="/opt/personal-site/frontend-v14-${release}"
...
tar -xzf /tmp/personal-site-frontend-v14.tar.gz -C "$frontend_new"
chown -R personalsite:personalsite "$frontend_new"
cp -a "$backend_current" "$backend_backup"
install -o personalsite -g personalsite -m 0644 /tmp/personal-site-backend-v14.jar "$backend_current"
systemctl restart personal-site-api

api_ready=false
for attempt in $(seq 1 60); do
  if curl --fail --silent http://127.0.0.1:8080/actuator/health >/dev/null \
      && curl --fail --silent 'http://127.0.0.1:8080/api/public/learning/logs' >/dev/null; then
    api_ready=true
    break
  fi
  sleep 1
...
echo "deployed=v14 release=${release}"
