...
new_frontend="/opt/personal-site/frontend-v9.1-${release}"
old_frontend="/opt/personal-site/frontend.${release}.bak"

mkdir -p "$new_frontend"
tar -xzf /tmp/personal-site-frontend-v9.1.tar.gz -C "$new_frontend"
...
for attempt in $(seq 1 20); do
  if curl --fail --silent http://127.0.0.1:3000/projects/ningbo-citizen-card-rights-confirmation \
      | grep --quiet '项目背景'; then
