...
new_frontend="/opt/personal-site/frontend-v8-${release}"

mkdir -p "$new_frontend"
tar -xzf /tmp/personal-site-frontend-v8.tar.gz -C "$new_frontend"
...
for attempt in $(seq 1 20); do
  if curl --fail --silent http://127.0.0.1:3000/lab >/dev/null; then
