...
frontend_new="/opt/personal-site/frontend-v10-${release}"
...
tar -xzf /tmp/personal-site-frontend-v10.tar.gz -C "$frontend_new"
chown -R personalsite:personalsite "$frontend_new"
cp -a "$backend_current" "$backend_backup"
install -o personalsite -g personalsite -m 0644 /tmp/personal-site-backend-v10.jar "$backend_current"
systemctl restart personal-site-api

api_ready=false
for attempt in $(seq 1 40); do
  if curl --fail --silent http://127.0.0.1:8080/actuator/health >/dev/null \
      && curl --fail --silent 'http://127.0.0.1:8080/api/public/search?q=AI' >/dev/null \
      && curl --fail --silent http://127.0.0.1:8080/api/public/games >/dev/null; then
    api_ready=true
    break
  fi
  sleep 1
...
for attempt in $(seq 1 30); do
  if curl --fail --silent http://127.0.0.1:3000/games >/dev/null \
      && curl --fail --silent 'http://127.0.0.1:3000/search?q=AI' >/dev/null \
...
echo "deployed=v10 release=${release}"
