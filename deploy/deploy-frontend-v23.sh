...
frontend_new="/opt/personal-site/frontend-v23-${release}"

mkdir -p "$frontend_new"
tar -xzf /tmp/personal-site-frontend-v23.tar.gz -C "$frontend_new"
...
for attempt in $(seq 1 30); do
  if curl --fail --silent http://127.0.0.1:3000/learn >/dev/null \
      && curl --fail --silent http://127.0.0.1:3000/learn/station-01 >/dev/null; then
    web_ready=true
    break
  fi
  sleep 1
...
echo "deployed=v23 release=${release}"
